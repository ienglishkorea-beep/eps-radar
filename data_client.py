...
def fetch_price_history(
    ...
    url = _build_url("eod_price", ticker=ticker)
    raw = _request_json(
        url,
        params={
...
def build_price_metrics(price_df: pd.DataFrame) -> Dict[str, Optional[float]]:
    ...
    close_col = "adjClose" if "adjClose" in df.columns and df["adjClose"].notna().sum() > 0 else "close"
    ...
    if "high" in df.columns and len(df) >= 2:
        high_window = df["high"].tail(PRICE_WINDOWS["high_52w"])
        high_52w = safe_float(pd.to_numeric(high_window, errors="coerce").max())
    elif len(series) > 0:
...
def fetch_fundamental_daily(
    ...
    url = _build_url("fundamental_daily", ticker=ticker)
    raw = _request_json(
        url,
        params={
            "startDate": start_dt.isoformat(),
    ...
    if not isinstance(raw, list) or not raw:
        return pd.DataFrame(columns=["date"] + list(TIINGO_DAILY_FIELDS.values()))
...
def extract_latest_daily_values(daily_df: pd.DataFrame) -> Dict[str, Optional[float]]:
    out: Dict[str, Optional[float]] = {k: None for k in TIINGO_DAILY_FIELDS.keys()}
    ...
    for logical_name, tiingo_code in TIINGO_DAILY_FIELDS.items():
        out[logical_name] = safe_float(latest.get(tiingo_code))
...
def fetch_fundamental_statements(
    ...
    url = _build_url("fundamental_statements", ticker=ticker)
    raw = _request_json(
        url,
        params={
            "startDate": start_dt.isoformat(),
...
def _flatten_statement_items(
    ...
    df["date"] = pd.to_datetime(df["date"], utc=True, errors="coerce").dt.tz_convert(None)
    df = df.sort_values(["date", "section", "dataCode"]).reset_index(drop=True)
    return df
...
def extract_statement_series(
    ...
    tiingo_code = TIINGO_STATEMENT_FIELDS.get(logical_name)
    if not tiingo_code:
        return pd.DataFrame(columns=["date", "year", "quarter", "value"])

    subset = statements_df[statements_df["dataCode"] == tiingo_code].copy()
    if subset.empty:
        return pd.DataFrame(columns=["date", "year", "quarter", "value"])
...
def extract_latest_statement_values(
    statements_df: pd.DataFrame,
) -> Dict[str, Optional[float]]:
    out: Dict[str, Optional[float]] = {k: None for k in TIINGO_STATEMENT_FIELDS.keys()}
...
def load_universe_csv(path: str) -> pd.DataFrame:
    ...
    if missing:
        raise RuntimeError(f"유니버스 파일에 필요한 컬럼이 없다: {sorted(missing)}")
    ...
    df["name"] = df["name"].astype(str).where(df["name"].notna(), None)
    df["sector"] = df["sector"].astype(str).where(df["sector"].notna(), None)
