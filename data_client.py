...
def build_price_metrics(price_df: pd.DataFrame) -> Dict[str, Optional[float]]:
    ...
    close_col = (
        "adjClose"
        if "adjClose" in price_df.columns and price_df["adjClose"].notna().sum() > 0
        else "close"
    )
    ...
    ma_50 = (
        safe_float(series.rolling(PRICE_WINDOWS["ma_50"]).mean().iloc[-1])
        if len(series) >= PRICE_WINDOWS["ma_50"]
        else None
    )
    ma_200 = (
        safe_float(series.rolling(PRICE_WINDOWS["ma_200"]).mean().iloc[-1])
        if len(series) >= PRICE_WINDOWS["ma_200"]
        else None
    )
    ...
    if "high" in price_df.columns:
        high_52w = safe_float(
            pd.to_numeric(
                price_df["high"].tail(PRICE_WINDOWS["high_52w"]),
                errors="coerce",
            ).max()
        )
    if high_52w is None:
...
def _flatten_statement_items(raw_statements: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    for entry in raw_statements:
        statement_type_raw = entry.get("statementType")
        ...
        base_info = {
            ...
            "year": year,
            "quarter": period,
            "statement_type": statement_type_raw,
    ...
    df["quarter_num"] = pd.to_numeric(df["quarter_num"], errors="coerce")

    # 분기/연간 구분 보조 플래그
    st = df["statement_type"].astype(str).str.lower()
    q_raw = df["quarter"].astype(str).str.lower()

    df["is_quarterly"] = (
        st.str.contains("quarter")
        | q_raw.str.contains("q")
        | df["quarter_num"].isin([1, 2, 3, 4])
    )

    df["is_annual"] = (
        st.str.contains("annual")
        | st.str.contains("year")
        | q_raw.str.contains("fy")
        | q_raw.str.contains("annual")
    )

    # 최종 값 선택
    df["final_value"] = df["value"]
    df.loc[df["final_value"].isna(), "final_value"] = df["reportedValue"]
    df.loc[df["final_value"].isna(), "final_value"] = df["rawValue"]

    return df.sort_values(
        ["dataCode", "date", "year", "quarter_num"]
    ).reset_index(drop=True)


def _prepare_quarterly_series(series_df: pd.DataFrame) -> pd.DataFrame:
    if series_df.empty:
        return pd.DataFrame(columns=["date", "year", "quarter_num", "value"])

    work = series_df.copy()

    # quarterly 우선
    if "is_quarterly" in work.columns:
        quarterly = work[work["is_quarterly"] == True].copy()
        if not quarterly.empty:
            work = quarterly

    work = work.dropna(subset=["final_value"]).copy()
    work = work.dropna(subset=["year", "quarter_num"]).copy()

    # 같은 year/quarter 중복 제거: 가장 최근 date 우선
    work = work.sort_values(["year", "quarter_num", "date"])
    work = work.drop_duplicates(subset=["year", "quarter_num"], keep="last")

    work = work.rename(columns={"final_value": "value"})
    work = work[["date", "year", "quarter_num", "value"]].copy()
    work = work.sort_values(["year", "quarter_num"]).reset_index(drop=True)
    return work
...
def extract_statement_series(statements_df: pd.DataFrame, logical_name: str) -> pd.DataFrame:
    ...
    return _prepare_quarterly_series(subset)
...
def yoy_growth_latest(series_df: pd.DataFrame) -> Optional[float]:
    if series_df.empty or len(series_df) < 5:
        return None

    work = series_df.sort_values(["year", "quarter_num"]).reset_index(drop=True)
    latest = safe_float(work["value"].iloc[-1])
    prev_yoy = safe_float(work["value"].iloc[-5])

    if latest is None or prev_yoy in [None, 0]:
        return None

    return (latest / prev_yoy) - 1.0


def yoy_growth_prior(series_df: pd.DataFrame) -> Optional[float]:
    if series_df.empty or len(series_df) < 6:
        return None

    work = series_df.sort_values(["year", "quarter_num"]).reset_index(drop=True)
    prior = safe_float(work["value"].iloc[-2])
    prior_yoy = safe_float(work["value"].iloc[-6])

    if prior is None or prior_yoy in [None, 0]:
        return None

    return (prior / prior_yoy) - 1.0


def acceleration_score_inputs(series_df: pd.DataFrame) -> Dict[str, Optional[float]]:
    latest_growth = yoy_growth_latest(series_df)
    prior_growth = yoy_growth_prior(series_df)

    acceleration = None
    if latest_growth is not None and prior_growth is not None:
        acceleration = latest_growth - prior_growth
...
def compute_calculated_metrics(
    ...
    capex_accel = acceleration_score_inputs(capex_df)

    revenue_growth = revenue_accel["latest_growth"]
    ebit_growth = ebit_accel["latest_growth"]
    cfo_growth = cfo_accel["latest_growth"]
    capex_growth = capex_accel["latest_growth"]
...
def load_universe_csv(path: str) -> pd.DataFrame:
    ...
    if "ticker" not in df.columns:
        raise RuntimeError("유니버스 파일에 ticker 컬럼이 없다.")
