...
def _clean_string(value: Any) -> Optional[str]:
    ...
    bad_markers = [
        "field not available for free/evaluation",
        "field not available",
        "not available for free/evaluation",
        "not available",
        "n/a",
        "none",
        "null",
...
def _latest_dated_record(records: List[dict]) -> Optional[dict]:
    ...
    date_keys = ["date", "asOfDate", "reportDate", "calendarDate", "statementDate", "filingDate"]
...
def _search_str_in_records(records: List[dict], keys: List[str]) -> Optional[str]:
    if not records:
        return None
...
def _extract_sector_industry(daily_meta: dict, fund_meta: dict) -> Tuple[Optional[str], Optional[str]]:
    payload = {
...
def _extract_market_cap(daily_meta: dict, fund_meta: dict, fund_metrics: dict) -> Optional[float]:
    payload = {
        "daily_meta": daily_meta or {},
        "fund_meta": fund_meta or {},
    ...
    found = _search_number_in_records(
        metric_rows,
        ["marketCap", "market_cap", "marketcap", "marketCapitalization", "market_capitalization"],
    )
    if found is not None:
        return found
...
def _statement_line_keys() -> Dict[str, List[str]]:
    return {
        "revenue": ["revenue", "totalRevenue", "total_revenue", "revenues", "salesRevenueNet", "sales_revenue_net"],
        "gross_profit": ["grossProfit", "gross_profit", "grossIncome", "gross_income"],
        "operating_income": ["operatingIncome", "operating_income", "incomeFromOperations", "operatingProfit", "ebit"],
    }


def _extract_financial_base_fields(fund_stmt: dict) -> Dict[str, Optional[float]]:
    ...
    gross_profit_latest = _latest_dated_record(stmt_rows)
    gross_profit_value = None
    operating_income_value = None

    if gross_profit_latest is not None:
        for k in keys["gross_profit"]:
            gross_profit_value = safe_float(gross_profit_latest.get(k))
            if gross_profit_value is not None:
                break
        for k in keys["operating_income"]:
            operating_income_value = safe_float(gross_profit_latest.get(k))
    ...
    gross_margin = None
    if gross_profit_value is not None and revenue_latest not in [None, 0]:
        gross_margin = gross_profit_value / revenue_latest

    operating_margin = None
    if operating_income_value is not None and revenue_latest not in [None, 0]:
        operating_margin = operating_income_value / revenue_latest

    return {
        "revenue_latest": revenue_latest,
        "revenue_prior_yoy_base": revenue_prior,
        "revenue_growth": revenue_growth,
        "gross_profit_latest": gross_profit_value,
        "operating_income_latest": operating_income_value,
