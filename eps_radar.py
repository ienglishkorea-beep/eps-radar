...
MIN_REVISION_ABS = 0.005  # 0.5% 미만은 revision으로 무시
...
def compute_vcp_features(df: pd.DataFrame) -> Dict[str, Optional[float]]:
    ...
    tight_ok = tight_range_10d is not None and tight_range_10D_MAX if False else tight_range_10d <= TIGHT_RANGE_10D_MAX
...
def main() -> None:
    ...
    history["revision_pct"] = history["revision_pct"].apply(
        lambda x: None if x is None or pd.isna(x) or abs(float(x)) < MIN_REVISION_ABS else x
    )
