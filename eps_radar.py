...
def build_detail_comment(row: Dict[str, Any]) -> str:
    blocks: List[str] = []
    ...
    if revenue_growth is not None:
        if revenue_acc is not None:
            blocks.append(
                f"매출 성장률 {fmt_pct(revenue_growth)}, 가속도 {fmt_pct(revenue_acc)}."
            )
        else:
    ...
    if cfo_growth is not None:
        if cfo_acc is not None:
            blocks.append(
                f"영업현금흐름 성장률 {fmt_pct(cfo_growth)}, 가속도 {fmt_pct(cfo_acc)}."
            )
        else:
...
def evaluate_row(base_row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    passed, reason = pass_hard_filters(base_row)
    if not passed:
        return None
    ...
    detail_comment = build_detail_comment(base_row)
    if detail_comment:
