...
# 8) RS line high
# 9) Tight structure / VCP readiness
# 10) Quality Growth 태그
#
# 출력 등급
# - ULTRA
# - STRONG
...
#
# v2.2 변경
# - multibagger_potential -> quality_growth
# - is_ultra / is_quality_growth 출력 추가
# - quality_growth_candidates.csv 생성
# - 엔트리/스탑/20일고점 출력 제거
# - Yahoo 요청 retry 추가
# - 데이터 누락 진단 출력 추가
# - revision_count 2/3/4 감점 완화
# =========================================================
...
def fetch_json_with_retry(url: str, headers: Optional[dict] = None, timeout: int = 20) -> Optional[dict]:
...
def get_quote_summary_modules(ticker: str) -> Optional[dict]:
    ...
    data = fetch_json_with_retry(url, headers=USER_AGENT, timeout=20)
    if not data:
        return None
...
def get_growth_accel_proxy(
    ...
    eps_part = 1.0 if revision_count in [2, 3, 4] else 0.8
...
def get_growth_accel_tag(
    ...
    if (
        ...
        return "ACCEL"
    elif revision_count in [2, 3, 4] and revenue_growth >= 0.15:
        return "EARLY"
    return "NORMAL"
...
def compute_score(
    ...
    eps_score = 1.0 if revision_count in [2, 3, 4] else 0.8
...
def build_candidate_row(
    ...
    if sector_etf is None:
        diag["sector_map_missing_count"] += 1
        return None
    ...
    if sector_ret_6m is None or sector_ret_3m is None:
        diag["sector_return_missing_count"] += 1
        return None
    ...
    if (
        ...
        diag["quote_field_missing_count"] += 1
        return None
    ...
    if revenue_growth is None:
        diag["revenue_growth_missing_count"] += 1
        return None
    ...
    quality_growth = get_quality_growth_flag(
        revenue_growth=revenue_growth,
        gross_margin=gross_margin,
        operating_margin=operating_margin,
    ...
    row = {
        ...
        "is_ultra": False,
        "is_quality_growth": quality_growth,
...
def enrich_with_industry_and_grade(final_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in final_df.iterrows():
        ...
        quality_growth_flag = bool(row["quality_growth"])
        quality_growth_flags.append(quality_growth_flag)
    ...
    final_df["is_quality_growth"] = quality_growth_flags
...
def save_empty_outputs() -> None:
    ...
    empty.to_csv("quality_growth_candidates.csv", index=False)
...
def main() -> None:
    ...
    rows = []
    summary_missing_early = 0

    for ticker in tickers:
        summary = get_quote_summary_modules(ticker)
        if summary is None:
            summary_missing_early += 1
    ...
    if stage1.empty:
        ...
        print(f"Summary missing (initial EPS pull): {summary_missing_early}")
        print("Stage1 raw: 0")
        print("Final candidates: 0")
        print("Ultra candidates: 0")
        print("Quality Growth candidates: 0")
    ...
    diag = {
        ...
        "sector_map_missing_count": 0,
        "sector_return_missing_count": 0,
        "quote_field_missing_count": 0,
        "revenue_growth_missing_count": 0,
    ...
    quality_growth_df = final_df[final_df["is_quality_growth"] == True].copy()
    ...
    quality_growth_df.to_csv("quality_growth_candidates.csv", index=False)
    ...
    print(f"Quality Growth candidates: {len(quality_growth_df)}")
