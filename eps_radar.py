...
# =========================================
# FINAL EPS + MOMENTUM + SECTOR + GUIDANCE RADAR
# DETECTION-FIRST / ULTRA / MULTIBAGGER VERSION
# =========================================
...
REV_SPEED_THRESHOLD = 2                 # 30일 내 2회 상향이면 REV_ACCEL
REQUEST_SLEEP = 0.10
...
def get_estimate_change_proxy(ticker: str):
    try:
        url = (
            f"https://query2.finance.yahoo.com/v10/finance/quoteSummary/"
            f"{ticker}?modules=earningsTrend"
        ...
        for trend in trends:
            if trend.get("period") in ["+1y", "0y"]:
                target = trend
                break

        if target is None and trends:
            target = trends[0]

        if target is None:
            return None

        eps_trend = target.get("epsTrend", {})
        rev_est = target.get("revenueEstimate", {})
        ...
        eps_current = raw_of(eps_trend, "current")
        eps_30 = raw_of(eps_trend, "30daysAgo")
        eps_90 = raw_of(eps_trend, "90daysAgo")

        rev_current = raw_of(rev_est, "current")
        rev_30 = raw_of(rev_est, "30daysAgo")
        rev_90 = raw_of(rev_est, "90daysAgo")
...
def extract_news_titles(news_items):
    titles = []
...
def detect_guidance_from_titles(titles):
    joined = " || ".join([t.lower() for t in titles])

    if any(k in joined for k in GUIDANCE_NEGATIVE_KEYWORDS):
        return "DOWN", "DIRECT"
    if any(k in joined for k in GUIDANCE_POSITIVE_KEYWORDS):
        return "UP", "DIRECT"
    if any(k in joined for k in GUIDANCE_FLAT_KEYWORDS):
        return "FLAT", "DIRECT"

    return "UNKNOWN", "NONE"


def detect_guidance_proxy_from_estimates(proxy_dict):
    if proxy_dict is None:
        return "UNKNOWN", "NONE"

    eps_current = proxy_dict.get("eps_current")
    eps_30 = proxy_dict.get("eps_30")
    rev_current = proxy_dict.get("rev_current")
    rev_30 = proxy_dict.get("rev_30")

    ups = 0
    downs = 0

    if eps_current is not None and eps_30 is not None:
        if eps_current > eps_30:
            ups += 1
        elif eps_current < eps_30:
            downs += 1

    if rev_current is not None and rev_30 is not None:
        if rev_current > rev_30:
            ups += 1
        elif rev_current < rev_30:
            downs += 1

    if ups >= 2:
        return "PROXY_UP", "PROXY"
    if downs >= 2:
        return "PROXY_DOWN", "PROXY"

    return "UNKNOWN", "NONE"


def get_guidance_signal(ticker: str, estimate_proxy: dict):
    try:
        tk = yf.Ticker(ticker)
        news_items = getattr(tk, "news", [])
        titles = extract_news_titles(news_items)
    except Exception:
        titles = []

    signal, strength = detect_guidance_from_titles(titles)
    if signal != "UNKNOWN":
        return signal, strength

    proxy_signal, proxy_strength = detect_guidance_proxy_from_estimates(estimate_proxy)
    return proxy_signal, proxy_strength
...
def compute_score(
    ...
    guidance_bonus = 0.0
    if guidance_signal == "UP":
        guidance_bonus = 1.0
    elif guidance_signal == "PROXY_UP":
        guidance_bonus = 0.5
...
def build_candidates(stage1, rev_speed_df, spy_ret_6m, sector_returns, min_high_proximity):
    rows = []
    ...
    for _, row in stage1.iterrows():
        ...
        estimate_proxy = get_estimate_change_proxy(ticker)
        time.sleep(REQUEST_SLEEP)

        guidance_signal, guidance_strength = get_guidance_signal(ticker, estimate_proxy)
        time.sleep(REQUEST_SLEEP)
...
def main():
    ...
    final_df = build_candidates(stage1, rev_speed_df, spy_ret_6m, sector_returns, MIN_HIGH_PROXIMITY)

    if len(final_df) < AUTO_RELAX_IF_FINAL_LT:
        final_df = build_candidates(stage1, rev_speed_df, spy_ret_6m, sector_returns, RELAXED_HIGH_PROXIMITY)
    ...
    final_df["guidance_sort"] = final_df["guidance_signal"].map({
        "UP": 0,
        "PROXY_UP": 1,
        "FLAT": 2,
        "UNKNOWN": 3,
    ...
    final_df = final_df.sort_values(
        ...
        ascending=[True, True, True, True, True, False, True, False, False],
    ).reset_index(drop=True)

    final_df = final_df.drop(columns=["ultra_sort", "multi_sort", "growth_sort", "action_sort", "guidance_sort"])
