...
def compute_score(
    ...
    if sector_ret_6m is not None and ret_6m > sector_ret_6m > spy_ret_6m:
        trend_parts += 0.25
    elif ret_6m > spy_ret_6m:
        trend_parts += 0.10

    if sector_ret_3m is not None and ret_3m > sector_ret_3m > spy_ret_3m:
        trend_parts += 0.25
    elif ret_3m > spy_ret_3m:
        trend_parts += 0.10
...
def print_stage0_debug(stage0_rows: List[dict], sample_n: int = DEBUG_SAMPLE_N) -> None:
...
def build_candidate_row(
    ...
    sector_ret_6m = sector_returns.get(sector_etf, {}).get("ret_6m") if sector_etf else None
    sector_ret_3m = sector_returns.get(sector_etf, {}).get("ret_3m") if sector_etf else None

    if sector_etf is None:
        diag["sector_etf_missing"] += 1
    if sector_etf is not None and (sector_ret_6m is None or sector_ret_3m is None):
    ...
    tight_structure = bool(tight_range_10d is not None and tight_range_10D_MAX >= tight_range_10d)
    ...
    growth_accel_proxy = get_growth_accel_proxy(
        ...
        high_proximity=high_proximity,
        stock_ret_6m=ret_6m,
        sector_ret_6m=sector_ret_6m if sector_ret_6m is not None else spy_ret_6m,
        spy_ret_6m=spy_ret_6m,
        stock_ret_3m=ret_3m,
        sector_ret_3m=sector_ret_3m if sector_ret_3m is not None else spy_ret_3m,
        spy_ret_3m=spy_ret_3m,
        rs_line_high=rs_line_high,
    )
...
def enrich_with_industry_and_grade(final_df: pd.DataFrame) -> pd.DataFrame:
    ...
    working["industry_fallback"] = working["industry"].fillna("UNCLASSIFIED")

    industry_stats = (
        working.groupby("industry_fallback", dropna=False)
        .agg(
    ...
    breadth_rows = []
    for industry_name, sub in working.groupby("industry_fallback", dropna=False):
        up = (sub["price"] > sub["ma50"]).sum()
        total = len(sub)
        pct = up / total if total > 0 else None
        breadth_rows.append({"industry_fallback": industry_name, "industry_breadth_pct": pct})

    breadth_df = pd.DataFrame(breadth_rows)
    industry_stats = industry_stats.drop(columns=["industry_breadth_pct"]).merge(
        breadth_df,
        on="industry_fallback",
        how="left",
    )
    ...
    working = working.merge(industry_stats, on="industry_fallback", how="left", suffixes=("", "_y"))
    ...
    for _, row in working.iterrows():
        industry_top = bool(
            ...
            and row["industry_fallback"] != "UNCLASSIFIED"
        )
        ...
        score = compute_score(
            ...
            sector_ret_6m=row["sector_ret_6m"] if pd.notna(row["sector_ret_6m"]) else None,
            spy_ret_6m=float(row["spy_ret_6m"]),
            ret_3m=float(row["ret_3m"]),
            sector_ret_3m=row["sector_ret_3m"] if pd.notna(row["sector_ret_3m"]) else None,
    ...
    working["grade"] = grades
    working["score"] = scores
    working["is_ultra"] = ultra_flags
    working["is_multibagger"] = multibagger_flags

    grade_rank = {"ULTRA": 0, "STRONG": 1, "WATCH": 2}
    growth_rank = {"ACCEL": 0, "EARLY": 1, "NORMAL": 2}
    action_rank = {"BUY": 0, "WATCH": 1, "NO_ENTRY": 2}

    working["grade_sort"] = working["grade"].map(grade_rank).fillna(9)
    working["growth_sort"] = working["growth_accel_tag"].map(growth_rank).fillna(9)
    working["action_sort"] = working["action"].map(action_rank).fillna(9)
    ...
    working = working.drop(columns=["grade_sort", "growth_sort", "action_sort", "industry_fallback"])
    return working
...
def main() -> None:
    ...
    print_stage0_debug(stage0_rows, sample_n=DEBUG_SAMPLE_N)
