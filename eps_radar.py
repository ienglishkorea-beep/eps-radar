...
TARGETS: Dict[str, List[str]] = {
    ...
    "operating_margin": ["operating margin", "ebit margin", "ebita margin", "op margin"],
    "net_margin": ["net income margin", "net margin"],
    "roic": ["roic", "return on invested capital", "return on capital"],
    "pe": ["pe ratio", "p/e", "price earnings", "price / earnings"],
...
# 문서/로그에서 이미 확인됐거나 강하게 의심되는 code
KNOWN_CODES: Dict[str, List[str]] = {
    ...
    "cfo": ["cashFromOperations", "operatingCashFlow"],
...
def get_json(url: str, token: str, params: Optional[Dict[str, Any]] = None) -> Any:
    ...
    resp = requests.get(url, headers=headers, params=final_params, timeout=REQUEST_TIMEOUT)
...
def summarize_presence(
    ...
    for target_name, defs in matched_definitions.items():
        ...
        search_terms = candidate_codes + candidate_names + TARGETS[target_name] + KNOWN_CODES.get(target_name, [])
        ...
        report[target_name] = {
            ...
            "known_code_candidates": KNOWN_CODES.get(target_name, []),
    ...
    for target_name, info in report.items():
        ...
        if info["candidate_dataCodes"]:
            ...
            for x in info["candidate_dataCodes"][:10]:
                print(f"    - {x}")
        if info["known_code_candidates"]:
            print("  known_code_candidates:")
            for x in info["known_code_candidates"][:10]:
                print(f"    - {x}")
        if info["daily_hits"]:
            print("  daily_hits:")
