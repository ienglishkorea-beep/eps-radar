...
from data_client import (
    ...
    build_feature_row,
    build_price_metrics,
    extract_latest_daily_values,
    fetch_fundamental_daily,
    fetch_fundamental_statements,
    fetch_price_history,
    load_universe_csv,
    _flatten_statement_items,
)
...
# STAGED HARD FILTERS
# =========================================================
def pass_price_hard_filter(price_metrics: Dict[str, Any]) -> Tuple[bool, str]:
...
def pass_marketcap_hard_filter(
    ...
    if market_cap is None:
        return False, "시가총액 데이터 부족"
    if market_cap < MIN_MARKET_CAP:
        return False, "최소 시가총액 미달"
    return True, ""


def pass_final_required_fields(row: Dict[str, Any]) -> Tuple[bool, str]:
...
def evaluate_row(base_row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    passed, _ = pass_final_required_fields(base_row)
    if not passed:
        return None
...
def build_all_rows(universe_df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    for _, r in universe_df.iterrows():
        ...
        try:
            # 1) PRICE ONLY
            price_df = fetch_price_history(ticker=ticker)
            price_metrics = build_price_metrics(price_df)
            ok, reason = pass_price_hard_filter(price_metrics)
            if not ok:
                print(f"[SKIP] {ticker} | {reason}")
                continue

            # 2) DAILY ONLY
            daily_df = fetch_fundamental_daily(ticker=ticker)
            latest_daily = extract_latest_daily_values(daily_df)
            ok, reason = pass_marketcap_hard_filter(price_metrics, latest_daily)
            if not ok:
                print(f"[SKIP] {ticker} | {reason}")
                continue

            # 3) STATEMENTS LAST
            raw_statements = fetch_fundamental_statements(ticker=ticker)
            ...
            if final_row is not None:
                ...
                print(f"[SKIP] {ticker} | 최종 필수 데이터 부족")
